...
def add_product_seller(request):
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        isOnSale = request.POST.get('isOnSale')
        ...
        for f in request.FILES.getlist('myFile'):
            a = add_product_seller_to_database(product_name=product_name, product_description=product_description, product_category=category, price=product_price, current_Price=current_product_price, product_images=f, username=request.user, item_id=item_id, isOnSale=isOnSale)
            a.save()
...
def update_item_seller(request):
    if request.method == 'POST' and request.user.is_authenticated:
        try:
            ...
            a = add_product_seller_to_database.objects.filter(item_id=item_id).update(product_name=product_name, product_description=product_description, product_category=category, price=product_price, current_Price=current_product_price, product_images=images, isOnSale=isOnSale)
            # a.save()
            return redirect('/seller/index')
